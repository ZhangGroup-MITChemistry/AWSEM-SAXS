...
# File to make check the error for alpha simulation, makes file called error.dat with the sum error and the standard deviation of each bin
# Useage: python calc_error.py colvars_file pr_file
# colvars_file- colvars output, pr_file- experimental p(r)
# NOTE: make sure the number of collective varialbes that are NOT p(r) is the same as the collective_vars variable


numpy.set_printoptions(threshold=numpy.nan)
if len(sys.argv)!=3:
    print("Incorrect usage. Please input proper files:")
    print("python calc_error.py colvars_file pr_file")
    print("Quiting")
    exit()
...
f_sim=colvars_data[:,1:(len(colvars_data[1,:])-(collective_vars))]
